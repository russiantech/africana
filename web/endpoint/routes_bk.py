...
from web.models import Cate, Expenses, Item, StockHistory, Notification, Sales
            ...
            if salesform.validate_on_submit(): #same-as-post-request
                ...
                if (instock.in_stock < salesform.pcs.data):
                    if instock.dept == 'k':
                        ...
                        url_ = referrer   
                    return jsonify({ 
                        'response': f'There\'s only <b>({instock.in_stock})</b>, {instock.name} left, you\'ve probably sold ({salesform.pcs.data})</b> \
                ...
                if (it and salesform.is_update.data == True):
                    it.salez.in_stock += it.qty #add-back-deducted-qty-when-sales-was-recorded
                    it.qty = salesform.pcs.data #record-as-new-sales
...
def history_action(item_id=None, history_id=None, action=None):
    ...
    it = db.session.query(StockHistory).join(Item, StockHistory.item_id==Item.id).filter( and_(StockHistory.id == history_id, StockHistory.deleted == False) ).first()
    ...
    if request.method == 'DELETE' and it != None and action == 'del':
        ...
        db.session.flush()
        #db.session.refresh()
        flash(f'Success, {it.item.name} Deleted!', 'danger')
        return jsonify({ 
            'response': f'Hey!! you\'ve deleted a backup of {it.item.name}',
            'flash':'alert-danger',
            'link': f'{referrer}'})
    
    return jsonify({ 
        'response': f'Hey stop!!!. This endpoint is only designed to remove unwanted stock history backups',
        'flash':'alert-danger',
        'link': f'{referrer}'})
